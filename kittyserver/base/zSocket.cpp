...
#define success_unpack() \
	do { \
		...
		{ \
			...
			if (_rcv_raw_size >= nRecordLen/* && _rcv_queue.rd_size() >= nRecordLen*/ && nRecordLen > PH_LEN) \
			{ \
 ...
 * 			返回整数，表示实际接收的字节数
 */
int zSocket::recvToCmd(void *pstrCmd, const DWORD nCmdLen, const bool wait)
{
 ...
 * 			返回整数，表示实际发送的字节数
 */
int zSocket::sendRawData(const void *pBuffer, const DWORD nSize)
{
	int retcode = 0;
    ...
    logTime.now();
    //logMessage(((char*)(pBuffer)+sizeof(DWORD)),nSize-sizeof(DWORD),logTime.msecs(),true);
...
do_select:
	...
	if (retcode > 0 && DWORD(retcode) < nSize)
 ...
 * \return 发送数据是否成功
 */
bool zSocket::sendRawDataIM(const void *pBuffer, const DWORD nSize)
{
	...
	while(DWORD(offset) < nSize);

	return (DWORD(offset) == nSize);
}
 ...
 * \return 发送是否成功
 */
bool zSocket::sendCmd(const void *pstrCmd, const DWORD nCmdLen, const bool buffer)
{
 ...
 * \return 发送是否成功
 */
bool zSocket::sendCmdNoPack(const void *pstrCmd, const DWORD nCmdLen, const bool buffer)
{
	if (NULL == pstrCmd || nCmdLen <= 0)
		return false;
...
const char *zSocket::getIPByIfName(const char *ifName)
{
	...
	{
        Fir::logger->debug("[获取IP地址] (%s,%s)",ifName,ifr.ifr_name);
...
int zSocket::recvToBuf_NoPoll(void *pstrCmd, const DWORD nCmdLen)
{
	int retcode = 0;
 ...
 * 			返回整数，表示实际接收的字节数
 */
int zSocket::recvToCmd_NoPoll(void *pstrCmd, const DWORD nCmdLen)
{
	//够一个完整的纪录，不需要向套接口接收数据
	success_unpack();
 ...
 * 			返回整数，表示实际发送的字节数
 */
int zSocket::sendRawData_NoPoll(const void *pBuffer, const DWORD nSize)
{
    logTime.now();
    logMessage(((char*)(pBuffer)+sizeof(DWORD)),nSize-sizeof(DWORD),logTime.msecs(),true);
...
unsigned int zSocket::packetUnpack(unsigned char *in, const unsigned int nPacketLen, unsigned char *out)
{
	...
	if (PACKET_ZIP == ((*(unsigned int*)in) & PACKET_ZIP))
	{
        ...
        ret = qlz_decompress((char*)(&in[PH_LEN]),out,&state_decompress);
	}
	else
	{
		bcopy(&(in[PH_LEN]), out, nRecvCmdLen);
	}
    //Fir::logger->debug("[解压数据] (%u,%d,%u)",nPacketLen,iszip,ret);
    return ret;
}
...
unsigned int zSocket::packetUnpack(unsigned char *in, const unsigned int nPacketLen, unsigned char *out)
{
	...
	if (PACKET_ZIP == ((*(unsigned int*)in) & PACKET_ZIP))
	{
        ...
        std::vector<BYTE> inBuf(nRecvCmdLen);
        memmove(&inBuf[0],&in[PH_LEN],nRecvCmdLen);

        UncompressInc(outBuf,inBuf);
        memmove(out,&outBuf[0],outBuf.size());
...
unsigned int zSocket::packetUnpack(unsigned char *in, const unsigned int nPacketLen, unsigned char *out)
{
	...
	{
        iszip = true;
        size_t nUnzipLen = MAX_DATASIZE;
        SizeT recLen = nRecvCmdLen - LZMA_PROPS_SIZE;
        int retcode = LzmaUncompress(out,&nUnzipLen,&(in[PH_LEN + LZMA_PROPS_SIZE]),&recLen,&(in[PH_LEN]),LZMA_PROPS_SIZE);
		switch(retcode)
		{
			case SZ_OK:
				break;
			...
			case SZ_ERROR_DATA:
				{
					Fir::logger->fatal("(%s)SZ_ERROR_DATA." ,__PRETTY_FUNCTION__);
					return (unsigned int)-1;
				}
				break;
		}
